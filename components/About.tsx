import React from 'react'

export default function About() {
    return (
      <div>
        <div
          className=" flex items-center max-w-4xl mx-auto leading-10 text-justify"
          id="About"
        >
          <div className="text-gray-500 px-10 mt-10">
            <h1 className="text-center text-4xl text-white mb-10">About Me</h1>
  
            <p>
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolores
              suscipit quidem neque dolorem similique sapiente eveniet, sit
              pariatur, quam alias at ipsam! Illo mollitia dicta fugit voluptates
              consectetur nesciunt quo minima repudiandae unde, nisi culpa
              cupiditate, aliquam sint velit tenetur saepe hic ex nemo ipsa quae
              tempore natus. Commodi assumenda nesciunt ullam voluptatibus!
              Consectetur accusantium non odit blanditiis libero deleniti sequi
              accusamus cum, illo aliquam veniam dicta quas sit et, temporibus \
              quas inventore earum quibusdam quidem quos molestias similique
              dignissimos quo saepe, culpa exercitationem reiciendis blanditiis.
              Dolore perspiciatis neque nobis a tempora omnis asperiores vitae
              dolorum beatae. Suscipit qui dignissimos commodi illo similique,
              unde repudiandae a. Necessitatibus laborum sapiente voluptates.
            </p>
            <p>
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo esse
              delectus ipsum repudiandae, reprehenderit excepturi deserunt ex
              optio suscipit exercitationem veritatis hic. Quos veritatis odit hic
              fugiat doloremque aperiam magni quia facere culpa cupiditate? Autem
              iste nam fugiat quasi quisquam ut rem aperiam. Corrupti rerum natus
              aperiam, nulla quaerat reiciendis.
            </p>
          </div>
        </div>
      </div>
    );
  }